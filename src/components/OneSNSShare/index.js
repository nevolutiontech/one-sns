...
const OneSNSShare = () => {
  ...
  const GetSocialMediaSiteLinks_WithShareLinks = () => {
    var args = {};
    const validargs = [
      "url",
      "title",
  ...
  const GetSocialMediaSites_WithShareLinks_OrderedByPopularity = () => {
    return [
      "google.plus",
      "google.bookmarks",
      "facebook",
  ...
  return (
    <table>
      {socialmedia.map(socialmedium => {
        return (
          <tr>
            <td class="output-label">{socialmedium}</td>
            <td>{socialmediaurls[socialmedium]}</td>
          </tr>
        );
      })}
    </table>
